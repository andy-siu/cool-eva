...
export function decodeFrame(id: number, data: Buffer): DecodedValue[] {
  switch (id) {
    ...
    //
    // Energica's own VCU digital list, recovered from the service-tool executable, names both families
    ...
    // inside the service-tool executable (obd-garage/HEATED_GRIPS.md §3.0), and that table describes
    // every model the service tool serves rather than this one. A name off a table is not a
