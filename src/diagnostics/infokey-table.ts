...
// the service tool (Energica's dealer tool) carries a resource it calls `DTCInfoKeys`:
...
//   • `<Energica_Manuals>/the service tool/2021-version/files/the telemetry table`
//   • the `DTCInfoKeys` JSON embedded in the service-tool executable (2024 build), by way of
...
// It is the service tool's internal signal index and it has no meaning on the wire. The
// tempting reading `CommonIdentifier = 0x2000 | id` was tested against a real
// bank-2 dump and REFUTED (obd-garage/SERVICE_TOOL_FILES.md §2.3: width agreement is
// 31.7 % against a 39.0 % chance baseline, and a 29-long run of 2-byte signals
// cannot be placed anywhere in bank 2 at any offset). obd-garage/SERVICE_TOOL_2024.md
// §7.2 then found the reason in Energica's own source: `(bank << 12) | id` is
// real but applies to a DIFFERENT table, the service tool's `LiveData`, which carries its
...
// fields concatenated in infokey order is the service tool's own decoder behaviour as
