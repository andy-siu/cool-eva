...
// table the VCU runs, and Energica has shipped many: the 2024 the service tool build selects 28,
...
// resource NAME inside the service-tool executable, which is the one thing binding a table to a
...
export interface ParameterTableDelta {
  /** The number the VCU reports at parameters 276/277, and the `_<TABLE_TYPE>` resource name in the service-tool executable. */
  tableType: number;
...
/** Every `TABLE_TYPE` this software can describe a bike with, ascending. 28 in the 2024 the service tool build. */
  ...
  if (rebuilt !== delta.fingerprint) {
    ...
    throw new Error(
      `table-catalog: rebuilding table ${delta.tableType} from params.ecf plus its delta gives fingerprint ` +
        ...
        "guess. Regenerate with `node --experimental-strip-types scripts/extract-vcu-tables.ts <the service-tool executable>`."
    );
  }
  return {
