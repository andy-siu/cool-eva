...
//    the service-tool executable (`ControlMotorbikeOverview_AvailableActions.GetMotorbikeService()`,
...
//    if it is wrong is a negative response — which SERVICE_RESET.md §2 says the service tool
...
// "Sync RTC" is `KWP2000Moto.UpdateRTC()` in the service tool's shared library, and all it does is put ONE
...
// the service tool fires it automatically on every connect, after both diagnostic sessions
...
// occurs exactly once in the whole of the service tool's shared library, the `RoutinesID` enum has no clock
