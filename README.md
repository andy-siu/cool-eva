...
```bash
# 1. find the service-tool executable in your own Energica service-tool install. On Windows it is
#    typically C:\Program Files (x86)\Energica\the service tool\the service-tool executable — copy it
#    anywhere; the script only reads it.
# 2. regenerate the catalogue from it (works on macOS/Linux/Windows — no .NET tooling,
#    no the service tool install needed on the machine you run it on, just the file)
