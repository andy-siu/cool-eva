...
// Pulls Energica's VCU parameter tables out of the service-tool executable and writes them into
// src/vcu/table-catalog.data.ts, which is the file the service actually reads.
//
//     node --experimental-strip-types scripts/extract-vcu-tables.ts /path/to/the service-tool executable
//
...
// from the table the VCU is running. Energica ships many; the 2024 the service tool build has
...
//
// So: run this against your own the service tool install, commit the diff, and the refusal
...
// the service-tool executable is a .NET assembly and each table is a ZIP archive stored as a
...
// exe by 28 compiler-generated accessors (`strings the service-tool executable | grep '^get__[0-9]*$'`)
...
// repo runs on. Four of those stamps have no numeric resource name at all and the service tool
...
if (!exePath) {
  console.error(
    "usage: node --experimental-strip-types scripts/extract-vcu-tables.ts <the service-tool executable> [--stdout] [--replace]"
  );
  console.error("");
  console.error("the service-tool executable is Energica's service tool. A 2024-era install has it under");
  console.error("  C:\\Program Files (x86)\\Energica\\the service tool\\the service-tool executable");
...
if (tables.length === 0) {
  ...
  console.error(
    `extract-vcu-tables: found NO \`_<TABLE_TYPE>\` resources in ${exePath}, so nothing was written.\n` +
      "  • It has to be the service-tool executable ITSELF, not the installer. `energica-setup.msi` holds the exe inside a\n" +
      "    compressed CAB and nothing here can see through that — install it (or unpack the MSI) first.\n" +
      "  • If it IS the service-tool executable, this build stores its parameter bundles differently from the 2021 and 2024\n" +
